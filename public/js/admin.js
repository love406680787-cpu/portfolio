...
(function() {
    ...
    async function deleteFile(path, sha, message, branch = DATA_BRANCH) {
        return await githubRequest('DELETE', `/repos/${config.username}/${config.repo}/contents/${path}`, {
            message,
            sha,
            branch
        });
    ...
    async function loadWorksList() {
        ...
        try {
            ...
            list.innerHTML = works.map((work, i) => `
                <div class="work-item" data-index="${i}" data-path="${work.path || ''}" data-sha="">
                    <div class="work-item-thumb">
                        ${work.type === 'video'
                            ? `<video src="${work.url}" muted></video>`
                            : (work.url
                                ? `<img src="${work.url}" alt="${work.title}" loading="lazy">`
                                : `<span style="font-size:2rem;">${getTypeIcon(work.type)}</span>`)}
                    </div>
    ...
    // Make deleteWork available globally
    ...
    function initConfig() {
        ...
        // Prefill username from localStorage if saved
        const savedUsername = localStorage.getItem('portfolio_username');
        if (savedUsername && !config.username) {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            saveConfig();
            localStorage.setItem('portfolio_username', config.username);
            updateConfigStatus();
            showToast('配置已保存');

            // Try to init the works.json file if it doesn't exist
            try {
                const existing = await getFile(WORKS_FILE);
                if (!existing) {
                    await saveWorksToGitHub([]);
                    showToast('works.json 已创建，可以开始上传作品了！');
                }
            } catch (err) {
                // Maybe the repo doesn't exist yet
                showToast(`请确保仓库 ${config.username}/${config.repo} 已创建`, 'error');
            }
        });
    }
